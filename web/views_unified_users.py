...
from web.services.unified_user_service import (
    ...
    can_soft_disable_user,
    get_soft_disable_block_reason,
...
class UnifiedUserListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        devuelve filas ya encadenadas entre auth_user + funcionario + usuario + departamento.
        """
        qs = (
    ...
    def get_context_data(self, **kwargs):
        ...
        for link in ctx["page_obj"]:
            ...
            link.can_soft_disable = can_soft_disable_user(link.web_user)
            link.soft_disable_block_reason = get_soft_disable_block_reason(link.web_user)
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        return render(
            ...
            self.template_name,
            {
                ...
                "can_soft_disable": can_soft_disable_user(link.web_user),
                "soft_disable_block_reason": get_soft_disable_block_reason(link.web_user),
            },
        )
...
class UnifiedUserUpdateView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, pk, *args, **kwargs):
        link = get_object_or_404(
            FuncionarioWebUser.objects.select_related("web_user", "funcionario"),
            web_user_id=pk
        )
        u = link.web_user
...
class UnifiedUserDeleteView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        can_soft = can_soft_disable_user(u)
        block_reason = get_soft_disable_block_reason(u)
        ...
        return render(
            ...
            self.template_name,
            {
                ...
                "soft_disable_block_reason": block_reason,
            },
        )
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if hard_requested:
            ...
            messages.error(
                request,
                "❌ No se puede eliminar definitivamente porque el usuario tiene trazabilidad en denuncias."
            )
            return redirect("web:unified_user_delete", pk=u.pk)

        if not can_soft:
            messages.error(
                request,
                f"❌ No se puede desactivar este usuario. {block_reason}"
            )
            return redirect("web:unified_user_delete", pk=u.pk)
        ...
        if not can_hard:
            messages.warning(
                request,
                "⚠️ Tiene trazabilidad y no se puede eliminar. Se desactivó correctamente."
            )
        else:
            messages.info(request, "✅ Se desactivó correctamente (soft delete).")
